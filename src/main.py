...
2.   НКонфигурация: С помощью dataclass Config мы задаем все ключевые параметры в одном месте: пути к файлам, разделители, названия выходных папок. 
Это делает код гибким — чтобы адаптировать его под новые данные, достаточно поменять значения здесь.
...
@dataclass
class Config:
    csv_path: str = "data/dataset_travel.csv"   # путь к датасету
    sep: str = ";"
    encoding: str = "utf-8"
...
"""Здесь мы определяем все вспомогательные функции, которые будут многократно использоваться в проекте. 
Это хорошая практика, которая помогает избежать дублирования кода (Don't Repeat Yourself) и делает основной скрипт более читаемым.
